...
for idx, crime in enumerate(crime_types):
    ...
    merged_long = (
        merged_data.select(
            ["Socioeconomic_index", *rate_columns]
    ...
    for lvl in ["Low Opportunity", "Medium Opportunity", "High Opportunity"]:
        sub = trend.filter(pl.col("Socioeconomic_index") == lvl)
        ax_indiv.plot(
            sub["year"].to_list(),
    ...
    ax_indiv.set_ylabel(f"Average {crime.title()} Rate per 100K Residents")
...
#### Save figures ####
fig_combined.suptitle("Crime Rate Trends by SES Cluster (2019–2024)", fontsize=16)
